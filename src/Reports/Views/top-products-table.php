...
<div class="profitpress-table-wrap woocommerce-card">
	<h2 class="profitpress-table-title"><?php esc_html_e( 'Top 10 Most Profitable Products', 'profitpress' ); ?></h2>

	<?php if ( empty( $top_products ) ) : ?>
		<p class="profitpress-table-empty"><?php esc_html_e( 'No product sales in this period.', 'profitpress' ); ?></p>
	<?php else : ?>
		<table class="widefat striped profitpress-product-table">
			...
			<tbody>
				<?php foreach ( $top_products as $profitpress_row ) : ?>
					<tr>
						<td>
						...
						<td class="profitpress-num"><?php echo wp_kses_post( wc_price( (float) $profitpress_row['net_profit'], array( 'currency' => $currency ) ) ); ?></td>
						<td class="profitpress-num"><?php echo esc_html( (string) $profitpress_row['margin'] ); ?>%</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		</table>
